...
pub fn for_platform(comptime Platform: type) type {
    return struct {
        const main = @import("main.zig").for_platform(Platform);
        ...
        pub const MainLevel = struct {
            ...
            pub fn init(self: *Self, platform: *Platform, options: MainLevelOptions) void {
                const middle = (platform_mod.CANVAS_SIZE / 2) << 8;
                self.platform = platform;
                self.players[0] = Player.create((platform_mod.CANVAS_SIZE / 3) << 8, middle, 3, options.p1_behavior, &self.bullets[0]);
                self.players[1] = Player.create((platform_mod.CANVAS_SIZE * 2 / 3) << 8, middle, 2, options.p2_behavior, &self.bullets[1]);
            ...
            pub fn update(self: *Self) ?main.LevelInitializer {
                if (self.gameover and (self.platform.get_gamepad(.gamepad1) & (platform_mod.BUTTON_1 | platform_mod.BUTTON_2)) != 0) {
                    return .splash_level;
                }
                ...
                for (self.players) |*mp, i| {
                    if (mp.*) |*p| {
                        if (!self.gameover and p.check_collisions()) {
                            self.platform.tone(500, (16 << 24) | 38, 15, platform_mod.TONE_TRIANGLE);
                ...
                if (live_player_count == 1) {
                    if (!(self.players[live_player_idx orelse unreachable] orelse unreachable).is_human()) {
                        // Last player is AI
                        self.platform.text("Death!", 50, 80);
                        self.platform.text("X to restart", 30, 90);
                        self.gameover = true;
                    } else {
                        ...
                        const stayalive_frames_remaining = 3 * platform_mod.TARGET_FPS - @bitCast(i32, stayalive_frames_elapsed);
                        if (stayalive_frames_remaining > 0 and stayalive_frames_elapsed > 0) {
                            const seconds_remaining = @divTrunc(stayalive_frames_remaining, platform_mod.TARGET_FPS) + 1;
                            var buf: [20:0]u8 = undefined;
                            _ = std.fmt.bufPrintZ(&buf, "Stay Alive! {}", .{seconds_remaining}) catch unreachable;
                            self.platform.text(&buf, 20, 80);
                        } else if (stayalive_frames_remaining <= 0) {
                            self.platform.text("Victory!", 50, 80);
                            self.platform.text("X to restart", 30, 90);
                            self.gameover = true;
                        }
                    }
                } else if (live_player_count == 0) {
                    self.platform.text("Draw!", 50, 80);
                    self.platform.text("X to restart", 30, 90);
                    self.gameover = true;
                }
        ...
        pub const HumanPlayerBehavior = struct {
            const Self = @This();
            platform: *Platform,
            gamepad_id: platform_mod.GamepadId,

            fn get_gamepad(self: *Self) u8 {
                return self.platform.get_gamepad(self.gamepad_id);
            }
        };
        ...
        pub const RandomPlayerBehavior = struct {
            ...
            fn get_gamepad(self: *RandomPlayerBehavior) u8 {
                ...
                if (r == 0) {
                    return platform_mod.BUTTON_LEFT;
                }
                if (r == 1) {
                    return platform_mod.BUTTON_UP;
                }
                if (r == 2) {
                    return platform_mod.BUTTON_DOWN;
                }
                if (r == 3) {
                    return platform_mod.BUTTON_RIGHT;
                }
                return 0;
            }
        };
        ...
        pub const Player = struct {
            ...
            pub fn check_collisions(self: Self) bool {
                ...
                while (x < (startx + Self.width) and x < platform_mod.CANVAS_SIZE) : (x += 1) {
                    while (y < (starty + Self.height) and y < platform_mod.CANVAS_SIZE) : (y += 1) {
            ...
            pub fn create(x: u16, y: u16, draw_color: u8, behavior: PlayerBehavior, bullets: *[MainLevel.bullets_per_player]?Bullets) Self {
            ...
            pub fn update(self: *Self) void {
                ...
                if (just_pressed & platform_mod.BUTTON_LEFT != 0) {
                    self.fire(.LEFT);
                }
                if (just_pressed & platform_mod.BUTTON_RIGHT != 0) {
                    self.fire(.RIGHT);
                }
                if (just_pressed & platform_mod.BUTTON_UP != 0) {
                    self.fire(.UP);
                }
                if (just_pressed & platform_mod.BUTTON_DOWN != 0) {
                    self.fire(.DOWN);
                }
            ...
            fn draw(self: Self, platform: *Platform) void {
                platform.set_draw_colors(self.draw_color);
        ...
        fn Particles(comptime n: u32) type {
            return struct {
                ...
                fn update_and_draw(self: *Self, platform: *Platform) void {
                    var i: usize = 0;
                    platform.set_draw_colors(self.draw_color);
                    while (i < Self.n) : (i += 1) {
...
test "fire" {
    const Platform = @import("platform.zig").TestPlatform;
    const Module = for_platform(Platform);
    //var platform = Platform.create(.{});

    // Regression test for overflow
    var bullets: Module.Bullets = undefined;
    const middle = (platform_mod.CANVAS_SIZE / 2) << 8;
    var player = Module.Player.create((platform_mod.CANVAS_SIZE / 3) << 8, middle, 3, platform_mod.GAMEPAD1, &bullets);
    player.fire(.LEFT);
}
